...
// Localized navigation can change height; anchors follow the measured header.
const updateHeaderOffset=()=>document.documentElement.style.setProperty('--header-offset',(header.getBoundingClientRect().height+20)+'px');
new ResizeObserver(updateHeaderOffset).observe(header);updateHeaderOffset();
const revealHash=()=>{const target=document.getElementById(decodeURIComponent(location.hash.slice(1)));if(target)target.scrollIntoView({block:'start',behavior:'instant'});};
window.addEventListener('load',()=>{if(location.hash)requestAnimationFrame(revealHash);});
...
document.querySelectorAll('[data-tabs]').forEach(widget => {
  ...
  // A closed request is a result on every tab. Keep the original request DOM
  // available for replay, and reuse the result devices without changing decision location.
  if(controlCopy){
    panels.slice(0,-1).forEach(panel=>{
      [...panel.children].forEach(child=>child.dataset.openRequest='');
      [...panels.at(-1).children].forEach(child=>{
        const result=child.cloneNode(true);result.dataset.closedRequest='';result.hidden=true;
        result.querySelectorAll('[id]').forEach(node=>node.removeAttribute('id'));
        const explanation=result.querySelector('.ex-copy > p')|| (result.matches('.ex-copy')?result.querySelector('p'):null);
        if(explanation){for(const outcome of ['approved','declined','expired'])explanation.dataset[outcome]=controlCopy.closed[outcome];}
        panel.append(result);
      });
    });
  }
  ...
  function showDecision(decision) {
    ...
    widget.querySelectorAll('[data-open-request]').forEach(node=>node.hidden=decision!=='waiting');
    widget.querySelectorAll('[data-closed-request]').forEach(node=>node.hidden=decision==='waiting');
  ...
  function select(index, focus = false, decision, action = false, replay = false) {
    ...
    if(status)status.textContent=state.outcome==='waiting'?'':controlCopy.closed[state.outcome];
    if (action) {
      const heading=panels[index].querySelector('.ex-copy:not([hidden]) h3');
  ...
  widget.addEventListener('locale:restore', event => {
    const saved=event.detail;
    if(!saved || !Number.isInteger(saved.index) || saved.index<0 || saved.index>=tabs.length || !['waiting','approved','declined','expired'].includes(saved.outcome))return;
    if(saved.outcome!=='waiting')select(tabs.length-1,false,saved.outcome);
    select(saved.index);endHint();
  });
