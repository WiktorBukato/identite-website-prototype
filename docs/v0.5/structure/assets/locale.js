...
(()=>{
 const ttl=5*60*1000;
 const base=location.pathname.replace(/\/(en-US|es-MX|pt-BR)\/[^/]+$/,'');
 const key='identite.locale:'+base;
 const fieldList=()=>[...document.querySelectorAll('input,textarea,select')].filter(e=>!e.readOnly&&!e.disabled);
 const snapshot=()=>{
  const sections=[...document.querySelectorAll('main [id]')].filter(e=>e.getBoundingClientRect().height>0);
  const anchor=sections.filter(e=>e.getBoundingClientRect().top<=innerHeight*.35).pop()||document.querySelector('main');
  return {time:Date.now(),page:document.body.dataset.page,anchor:anchor?.id,offset:anchor?anchor.getBoundingClientRect().top/innerHeight:0,
 ...
 document.querySelectorAll('[data-locale]').forEach(a=>{
  ...
  a.addEventListener('click',event=>{
   ...
   try{const state=snapshot();state.target=url.pathname;sessionStorage.setItem(key,JSON.stringify(state));}catch{/* Navigation remains usable if session storage is unavailable. */}
  });
 });
 let saved;
 try{saved=JSON.parse(sessionStorage.getItem(key)||'null');sessionStorage.removeItem(key);}catch{return;}
 if(!saved||saved.target!==location.pathname||saved.page!==document.body.dataset.page||Date.now()-saved.time>ttl)return;
